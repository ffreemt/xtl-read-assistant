...
from deepl_tr_async.deepl_tr_async import LOOP, BROWSER
from deepl_tr_async.google_tr_async import google_tr_async as google_tr
from detect_lang_pg import detect_lang
from load_env import load_env

# LOOP = asyncio.get_event_loop()
arun = lambda _: LOOP.run_until_complete(_)  # noqa: E731

logger.info(" chrome.exe spawn: %s", BROWSER.process.pid)
logger.info(" In case o abnormal exit, do 'taskkill /f /pid %s'", BROWSER.process.pid)


def kill_browser(pid=BROWSER.process.pid):
    """
    clean the mess
    """
    err, out = sp.getstatusoutput(f" taskkill /f /pid {BROWSER.process.pid}")
    if err:
        logger.error("%s", err)
    else:
        logger.info("%s", out)
    logger.info("\n\t%s", " The force be with ya! ")
...
def on_activate(from_lang="auto", to_lang="auto", width=60):
    ...
    # _ = asyncio.gather(*coros)
    try:
        _ = asyncio.gather(*coros, loop=LOOP, return_exceptions=True)
    except Exception as exc:
    ...
    prefixes = ["dl: ", "gl: ", "dl: ", "gl: ", ]
    indent = " " * 4
    langs = ["zh", "zh", "en", "en"]

    outtext = ""
    for idx, elm in enumerate(res):
        if langs[idx] in ['zh', 'ja']:
            outtext_ = fill(prefixes[idx] + elm, width // 2, subsequent_indent=indent)
        else:
            outtext_ = fill(prefixes[idx] + elm, width, subsequent_indent=indent)
        outtext += outtext_ + "\n"
...
def main():
    ...
    try:
        with keyboard.Listener(
                ...
                on_press=lambda k: hotkey.press(listener.canonical(k)),  # type: ignore
        ...
        logger.error(" exc: %s of user interruprt", exc)
        logger.info("Goodbye!")
